...
function chkDupId(uid) {
	if (uid.length >= 4) {	// 사용자가 입력한 아이디가 4자 이상일 경우
		$.ajax({
			type : "POST", 				// 데이터 전송 방법
			url : "/GongU/dupId", 	// 데이터를 받을 서버의 URL로 컨트롤러(서블릿)를 의미
...
<div id="agreement" style="width:700px; height:100px; overflow:auto; border:1px solid black;">
공유해방 약관사항입니다. 근데 대부분 약관사항을 읽지를 않지요. 하지만 이것은 약관입니다.<br />
약관사항을 꼼꼼히 읽으신다면 정말 대단하다고 생각합니다. 대부분 안 읽고 동의부터 하니까요.
</div>
<p style="width:700px; text-align:center;">
	<input type="radio" name="agree" value="y" id="agreeY" checked="checked" /><label for="agreeY">동의</label>
	<input type="radio" name="agree" value="n" id="agreeN" /><label for="agreeN">미동의</label>
</p><br />

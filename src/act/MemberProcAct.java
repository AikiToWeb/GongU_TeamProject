...
public class MemberProcAct implements Action {
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		MemberInfo mi = null;
		// 정보 수정이나 탈퇴 시 사용할 세션 속성을 MemberInfo 형 인스턴스로 생성
		...
		int result = memberProcSvc.memberProc(wtype, memberInfo);
		// 가입, 수정, 탈퇴를 처리할 메소드 호출(memberAddr은 수정과 탈퇴 시 null로 가져감)
		...
		if (result == 1) {	// 정상적으로 동작되었으면
			if		 (wtype.equals("in"))		lnk = "../login_form.jsp";
				...
				// 정보 수정 성공 시 현재 세션에 들어있는 로그인 회원 정보도 변경시킴
			}
		} else {
			response.setContentType("text/html; charset=utf-8");
			PrintWriter out = response.getWriter();
			out.println("<script>");
			out.println("alert('작업에 실패했습니다. 다시 시도 해보세요.');");
